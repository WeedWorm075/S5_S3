    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (page.startsWith("list") && page.endsWith(".jsp")) {
                ...
                System.out.println("__className : "+className);

//                try {
                    Class<?> clazz = Class.forName("voyage." + className);
                    Object instance = clazz.newInstance();
                    java.lang.reflect.Method method = clazz.getMethod("all" + className, Connection.class);
                    System.out.println("__method : "+method.getName());

//                    try (Connection co = new Connexion_projet().getconnection()) {
                        Connection co = new Connexion_projet().getconnection();
                        List<?> result = (List<?>) method.invoke(instance, co);
                        
                        Bouquet b = new Bouquet();
                        List<Bouquet> lb = b.allBouquet(co);
                        
                        System.out.println("__tab size = "+result.size());
